...
public class ResourcesManager : Singleton<ResourcesManager> { 
    ...
    //做内存的释放有几种方式， 一种是简单的规定双向链表的最大缓存个数，一个是获取Android或ios的内存使用情况
    protected const int MAXCACHECOUNT = 0;
    ...
    /// </summary>
    public void WashOut() { 
        if (m_noRefDataItemDLMap.GetCount() > MAXCACHECOUNT)
        {
            for (int i = 0; i < MAXCACHECOUNT / 2; i++)  //释放一半
            {
...
public class DoubleLinkMap<T> where T : class, new()
{
    ...
    void Clear() {
        while (m_doubleLinkList.Tail != null) {
